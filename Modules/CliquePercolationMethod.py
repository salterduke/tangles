...
def clique_percolation_method(graph, k = 3, workers = 1, attribute = None, verbose = False):
    ...
    # MSD addition:
    # was not correctly finding comms composed of a single clique if that clique id was beyond the
    # bounds of the v indicess found in edge_list, as v not created in clique_graph
    # for example if only one clique found
    # replaced: ----------------------------------------------------
    # clique_graph = Graph(edge_list)
    # with: --------------------------------------------------------
    clique_graph = Graph()
    clique_graph.add_vertices(list(range(0,num_cliques)))
    clique_graph.add_edges(edge_list)
